namespace CustomCode.Test.BehaviorDrivenDevelopment
{
    ...
    /// <summary>
    /// A validator that defines assertions for reference types.
    /// </summary>
    public struct ReferenceTypeValidator<T> : IFluentInterface
        where T : class
    {
        #region Dependencies
            ...
            [CallerMemberName] string testMethodName = null, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string sourceCodePath = null)
        {
            ...
            {
                var context = Context.GetCallerContext(testMethodName, null, sourceCodePath, lineNumber);
                throw Context.GetFormattedException(testMethodName, context, $"is \"{Value?.GetType()?.Name ?? "undefined"}\"", $"to be of type \"{typeof(TType).Name}\"", because);
            }
        }
